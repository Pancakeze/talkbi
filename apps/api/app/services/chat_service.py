...
def _validated_staging_tables(ds: DataSource) -> list[dict]:
    """
    Only trust staging metadata that matches server-created Excel table names
    for this exact data source. Client-created data sources may contain arbitrary
    JSON in connection_info, so chat must not use it as an executable SQL target.
    """
