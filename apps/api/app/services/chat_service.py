...
_STAGING_IDENTIFIER_RE = re.compile(r"^[a-zA-Z_][a-zA-Z0-9_]{0,62}$")
...
def _normalize_qualified_identifier(value: object) -> Optional[str]:
...
def _expected_qualified_identifier(dialect: str, physical: str) -> str:
    if dialect == "postgresql":
        return f"{STAGING_SCHEMA}.{physical}"
    return physical


def _valid_staging_tables(ds: DataSource) -> list[dict]:
    """
    Trust only staging metadata that matches tables generated by Excel upload.
    Generic data-source connection_info is client supplied, so chat execution must
    not derive SQL allowlists from it unless the table names/qualification match
    the server-generated ds_<data_source_id>_* convention exactly.
    """
    ...
    if not isinstance(staging, dict) or staging.get("dialect") != dialect:
        return []
    ...
    for raw_meta in tables_meta_list:
        if not isinstance(raw_meta, dict):
            continue

        physical = raw_meta.get("table")
        if (
            not isinstance(physical, str)
            or not physical.startswith(expected_prefix)
            or not _STAGING_TABLE_RE.fullmatch(physical)
        ):
            continue

        schema = raw_meta.get("schema")
        if dialect == "postgresql":
        ...
        qualified = _normalize_qualified_identifier(raw_meta.get("qualified"))
        if qualified != _expected_qualified_identifier(dialect, physical):
            continue
        ...
        for col in columns:
            if not isinstance(col, dict):
                continue
            name = col.get("name")
            if not isinstance(name, str) or not _STAGING_IDENTIFIER_RE.fullmatch(name):
                continue
            safe_columns.append({"name": name, "dtype": str(col.get("dtype", ""))})

        if not safe_columns:
            continue

        valid_meta = dict(raw_meta)
        valid_meta["columns"] = safe_columns
        valid_tables.append(valid_meta)
