...
def test_chat_rejects_client_supplied_staging_metadata(client: TestClient):
    login = client.post("/api/auth/login", json={"username": "admin", "password": "admin123"})
    token = login.json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}
                    ...
                    "tables": [
                        {
                            ...
                            "columns": [
                                ...
                                {"name": "hashed_password", "dtype": "str"},
                            ],
                        }
                    ]
                }
    ...
    theme = client.post(
        "/api/theme-libraries",
        json={"name": "伪造用户表主题库", "description": "", "data_source_id": forged.json()["id"]},
        headers=headers,
    )
    assert theme.status_code == 200, theme.text
    theme_id = theme.json()["id"]
    ...
    chat = client.post(
        "/api/chat/query",
        json={"prompt": "preview users", "theme_ids": [theme_id]},
        headers=headers,
    )
    assert chat.status_code == 200, chat.text
    data = chat.json()
