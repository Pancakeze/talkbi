...
def test_validate_sql_enforces_table_allowlist():
    ...
    with pytest.raises(ValueError):
        validate_sql('SELECT t.a FROM "staging"."ds_1_t" t, users u LIMIT 10', policy=policy)
    with pytest.raises(ValueError):
        validate_sql('SELECT (SELECT hashed_password FROM users LIMIT 1) AS h FROM "staging"."ds_1_t" LIMIT 10', policy=policy)
    with pytest.raises(ValueError):
        validate_sql('SELECT * FROM (SELECT * FROM "staging"."ds_1_t" LIMIT 10) x LIMIT 10', policy=policy)
...
def test_validate_sql_rejects_select_into():
    with pytest.raises(ValueError):
        validate_sql('SELECT a INTO "staging"."copy" FROM "staging"."ds_1_t" LIMIT 1')
