...
def test_validate_sql_rejects_tableless_query_with_allowlist():
    policy = SQLGuardPolicy(
        allowed_schemas=("staging",),
        allowed_tables=frozenset({"staging.ds_1_t"}),
    )
...
def test_validate_sql_rejects_comma_join_allowlist_bypass():
    ...
    with pytest.raises(ValueError):
        validate_sql(
            'SELECT a FROM "staging"."ds_1_t", "public"."users" LIMIT 10',
            policy=policy,
        )
...
def test_validate_sql_rejects_select_into():
    with pytest.raises(ValueError):
        validate_sql('SELECT a INTO copied FROM "staging"."ds_1_t" LIMIT 10')
...
def test_validate_sql_rejects_file_access_functions():
    with pytest.raises(ValueError):
        validate_sql("SELECT readfile('/etc/passwd') LIMIT 1")
    with pytest.raises(ValueError):
        validate_sql("SELECT pg_read_file('/etc/passwd') LIMIT 1")
