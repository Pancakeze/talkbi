...
def test_validate_sql_enforces_table_allowlist_for_comma_from():
    policy = SQLGuardPolicy(
        allowed_schemas=("staging",),
        allowed_tables=frozenset({"staging.ds_1_t"}),
    )
    with pytest.raises(ValueError):
        validate_sql('SELECT * FROM "staging"."ds_1_t", users LIMIT 10', policy=policy)
...
def test_validate_sql_enforces_schema_when_no_allowlist():
    ...
    with pytest.raises(ValueError):
        validate_sql('SELECT * FROM "staging"."ds_1_t", "public"."users" LIMIT 10', policy=policy)
