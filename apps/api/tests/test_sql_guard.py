...
def test_validate_sql_checks_comma_join_inside_cte():
    policy = SQLGuardPolicy(
        allowed_schemas=("staging",),
        allowed_tables=frozenset({"staging.ds_1_t"}),
    )
    ...
    with pytest.raises(ValueError):
        validate_sql(
            'WITH rows AS (SELECT t.a FROM "staging"."ds_1_t" t, users u LIMIT 10) '
            "SELECT * FROM rows LIMIT 10",
            policy=policy,
        )

    validate_sql(
        'WITH rows AS (SELECT t.a FROM "staging"."ds_1_t" t LIMIT 10) '
        "SELECT * FROM rows LIMIT 10",
        policy=policy,
    )
