...
def test_validate_sql_accepts_allowlisted_cte():
    policy = SQLGuardPolicy(
        allowed_schemas=("staging",),
        allowed_tables=frozenset({"staging.ds_1_t"}),
    )
    validate_sql(
        'WITH t AS (SELECT a FROM "staging"."ds_1_t") SELECT * FROM t LIMIT 10',
        policy=policy,
    )
...
def test_validate_sql_enforces_schema_when_no_allowlist():
    ...
    with pytest.raises(ValueError):
        validate_sql('SELECT a FROM "staging"."ds_1_t" t, "public"."users" u LIMIT 10', policy=policy)
...
def test_validate_sql_rejects_no_table_query_when_allowlist_is_required():
    policy = SQLGuardPolicy(allowed_tables=frozenset({"staging.ds_1_t"}))
    with pytest.raises(ValueError):
        validate_sql("SELECT current_setting('data_directory')", policy=policy)
